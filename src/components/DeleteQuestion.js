...
function DeleteQuestion(props){
  ...
  mutation MyMutation($id: ID = "ckyxb8zcw0wty0d578jhowzc2") {
  ...
  `
  const deleteAnswers = gql`
  mutation MyMutation($id: ID!) {

    deleteManyAnswersConnection(where: {question: {id: $id} }) {
      edges {
        node {
          id
        }
...
`
  ...
  const deleteAll = async()=>{
      ...
      await request(uri, deleteAnswers, variables, requestHeaders)
      await request(uri, deleteQuestion, variables, requestHeaders)       
    }
      return(
        deleteAll()
